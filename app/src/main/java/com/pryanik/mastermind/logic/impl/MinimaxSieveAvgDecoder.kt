...
class MinimaxSieveAvgDecoder(private val allAnswers: List<String>) : Decoder {
    ...
    private fun nextStep(
        ...
        // Чтобы уменьшить среднее число шагов ищем ответы только в available
        return ResultGuess(available.minByOrNull { g ->
            available.map { s ->
                evaluate(g, s)
            }.counting()
                .values.maxOrNull()!!
